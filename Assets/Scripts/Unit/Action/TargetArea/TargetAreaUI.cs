...
public class TargetAreaUI : ActionUI {

	Tile target;
	TargetAreaAction targetArea;
	...
	void Start()
	{
        ...
        */
		...
		Debug.Log("AoE targetting start, player pos " + unit.tile.coordinate.ToString() + " targetting " + target.coordinate.ToString());
	...
	public override void SubmitInput()
	{
		if (IsInRange(target.coordinate))
		{
			...
			Debug.Log("Submitted area targetted action, facing " + approxDir.ToString());
			unit.QueueAction(targetArea, approxDir, timeline);
	...
	private bool IsInRange(Vector2 coord)
	{
		...
		if (taxiDistance > targetArea.rangeMax || taxiDistance < targetArea.rangeMin)
		{
